document.addEventListener('DOMContentLoaded', () => {
    ...
    const searchInputField = searchButton ? searchButton.querySelector('input') : null;
    ...
    const sortIcon = document.querySelector('.advance-search .sort-selection .selection-input-field i');

    // Add focus and blur event listeners to the search input field

    if (searchInputField && searchButton) {
        searchInputField.addEventListener('focusin', function () {
    ...
    if (sortInputField && sortOptions) {
        sortInputField.addEventListener('click', function () {
    ...
    document.addEventListener('click', (e) => {
        if (sortInputField && sortOptions && e.target !== sortInputField && !sortInputField.contains(e.target)) {
    ...
    document.querySelectorAll('.advance-search .sort-selection .selection-options .opt').forEach(option => {
        option.addEventListener('click', function () {
            const sortValueInput = sortInputField ? sortInputField.querySelector('input') : null;
            if (sortValueInput) {
                sortValueInput.value = this.textContent;
            }
            if (sortOptions) {
                sortOptions.classList.remove('active');
            }
